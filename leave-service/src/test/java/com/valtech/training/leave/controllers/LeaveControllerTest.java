...
class LeaveControllerTest {
	...
	void testApplyAndApprove() {
		...
		LeaveVO vo =restTemplate.postForObject(url + "apply",new LeaveVO(0,"08-03-2025","09-03-2025",
				LeaveType.CASUAL.name(),LeaveStatus.APPLIED.name(),"Going to Native","",2L), LeaveVO.class);
		assertTrue(vo.id() !=0);
	...
	void test() {
		...
		if(leaves.size() == 0) {
			...
			restTemplate.postForObject(url,new LeaveVO(0,"12-03-2025","13-03-2025",
					LeaveType.SICK.name(),LeaveStatus.APPLIED.name(),"Have Fever","",2L), LeaveVO.class);
			
			restTemplate.postForObject(url,new LeaveVO(0,"08-03-2025","09-03-2025",
					LeaveType.CASUAL.name(),LeaveStatus.APPLIED.name(),"Going to Native","",3L), LeaveVO.class);
