...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	try(Connection conn = getConnection()){
		PreparedStatement ps = conn.prepareStatement
				("INSERT INTO EMPLOYEE (NAME,AGE,GENDER,SALARY,EXPERIENCE,LEVEL,ID,DEPT_ID) VALUES (?,?,?,?,?,?,?,?)");
	...
	try (Connection conn = getConnection()){
		PreparedStatement ps = conn.prepareStatement
				("SELECT ID,NAME,AGE,SALARY,EXPERIENCE,LEVEL,GENDER,DEPT_ID FROM EMPLOYEE WHERE ID =?");
		ps.setInt(1, id);
	...
	public Employee EmployeeByDeptID(int dept_id) {
		
		try(Connection conn = getConnection()){
			PreparedStatement ps = conn.prepareStatement("SELECT ID,NAME,AGE,SALARY,EXPERIENCE,LEVEL,GENDER FROM EMPLOYEE WHERE ID =?");
			ps.setInt(1, dept_id);
			ResultSet rs =ps.executeQuery();
			if(rs.next()) {
				Employee e = populateEmployee(rs);
				...
				new RuntimeException("No employees with dept_id "+dept_id+" Found.");
			}
		} catch(Exception ex) {
			throw new RuntimeException(ex);
		}
	...
	private Employee populateEmployee (ResultSet rs) throws SQLException {
		return Employee.builder().id(rs.getLong(1)).name(rs.getString(2)).age(rs.getInt(3))
				.gender(rs.getString(7)).salary(rs.getFloat(4)).exp(rs.getInt(5)).level(rs.getInt(6)).dept_id(rs.getInt(8)).build();
	}
	...
	public List<Employee> getAll() {
		...
		try(Connection conn = getConnection()) {
			PreparedStatement ps = conn.prepareStatement
					("SELECT ID,NAME,AGE,SALARY,EXPERIENCE,LEVEL,GENDER,DEPT_ID FROM EMPLOYEE");
			ResultSet  rs = ps.executeQuery();
			while(rs.next()) {
